...
Based on this intermediate dataset, the top 3 states with the highest fatalities are Sagaing, Magway(Magwe) and Mandalay, which earlier we have discussed to be the 'North-West' region of Myanmar.

According to [COAR Global](https://coar-global.org/2023/09/27/broken-heartland-armed-violence-and-emergency-response-in-central-myanmar/), and other organizations following the conflict such as the [Myanmar IISS](https://myanmar.iiss.org/analysis/humanitarian), this region has been afflicted with decades long ethinic and religious tensions, economic hardship.
...
As such, I will focus my analysis on these regions
...
```{r}
class(armed_conflict_quartered)
```

```{r}
armed_conflict_3_sf <- armed_conflict_quartered %>% 
  filter(admin1 %in% c("Sagaing", "Magway", "Mandalay"))
```

```{r}
state_region_3_sf <- state_region_sf %>% 
...
```{r fig.width=12, fig.height=10}
ggplot(armed_conflict_quartered, aes(fill = FATALITIES, geometry = geometry)) +
  geom_sf(size = 0.001, alpha = 0.5) +
  scale_fill_gradientn(
    colours = plasma(10), 
    breaks = c(0, 5, 10, 20, 30, 40, 50, 100, max(armed_conflict_quartered$FATALITIES)),
    limits = c(0, max(armed_conflict_quartered$FATALITIES)),
    oob = scales::squish
  ) +
  facet_wrap(~ quarter, ncol = 4, nrow = 4) +
  labs(title = "Total Fatalities by Event Type per Quarter", fill = "Fatalities") +
  theme_minimal()
