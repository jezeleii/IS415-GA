...
From the plots above, we see that armed conflict even types seem to concentrate in the North-west region, particularly at the boundary of Sagaing, Mandalay and Magwe state, which we will zoom in later on.
...
To see the total fatalities: (from the initial dataset)

```{r}
armed_conflict_fatalities <- armed_conflict %>% 
  group_by(admin1, event_type, sub_event_type) %>% 
...
### 3.2.1 Data Processing of subsetted dataset
...
From this point forward, we will focus on the event_type of 'Battles' and sub_event_type of 'Armed Clash' as it would be interesting to analyse given that it has the most event counts and fatalities as aggregated by the code chunk below:
...
```{r}
armed_clash_sf <- armed_conflict_quartered %>% 
  filter(event_type == "Battles", sub_event_type =="Armed clash") %>% 
  group_by(quarter, event_type, sub_event_type, admin1, geometry) %>% 
  summarize(
...
unique_coordinates <- armed_clash_sf %>%
  mutate(geometry_as_wkt = st_as_text(geometry)) %>%  
  summarise(unique_count = n_distinct(geometry_as_wkt))

total_armed_clash_count <- armed_clash_sf %>% 
  summarize(total_event_count = sum(event_count, na.rm = TRUE)) %>% 
  ungroup()
...
Why are there so many duplicates out of 11770 observations?

1.  **Multiple Events Occurring at the Same Location**
2.  **Spatial Granularity**
3.  **Time Dimension is Not Considered in Duplicates**:

Based on the analysis of unique coordinate points earlier, the unique coordinates total to 3265 unique locations within the initial dataset, so the `sum(multiplicity(armed_conflict_ppp) > 1)` result is actually misleading as it may be accounting for events across time in the same coordinate location.
...
Out of 3265 unique coordinate points, 1446 have more than one conflict events occuring throughout the time period of Jan 2021 - Jun 2024.
...
```{r}
armed_clash_myn_ppp <- armed_clash_ppp[myanmar_states_owin]
```
...
Before proceeding, we take a look at how the fatalities and event_counts are aggregated.

```{r}
armed_conflict_fatalities %>% filter(event_type == "Battles", sub_event_type == "Armed clash")
```
...
```{r}
kde_armed_clash_myn.ppl <- density(armed_clash_myn_ppp.km, 
                                   sigma=bw.ppl, 
                                   edge=TRUE,
                                   kernel="gaussian")

par(mfrow=c(1,2))
plot(kde_armed_clash_myn.bw, main="bw.diggle")
plot(kde_armed_clash_myn.ppl, main="bw.ppl")
```
...
Based on our earlier analysis, we received a sigma value of 0.1809693, which results in minimal smoothing, meaning the KDE will show more localized patterns, however, this could lead to overfitting, where small clusters are exaggerated and conflict points are more prone to misinterpretation.
...
par(mfrow=c(1,2))
kde_sigma <- density.ppp(armed_clash_myn_ppp.km, sigma=sigma_diggle)
kde_nearest_neighbour <- density.ppp(armed_clash_myn_ppp.km, sigma=mean_distance)
plot(kde_sigma)
plot(kde_nearest_neighbour)
```
...
+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| Initial Sigma Value (0.18)                                                                                                                                                           |                                                                                                                                                                                                       | Mean Distance (1.6)                                                                                                                                                                                                                                                                                                                                                        |                                                                                                                                                                       |
+======================================================================================================================================================================================+=======================================================================================================================================================================================================+============================================================================================================================================================================================================================================================================================================================================================================+=======================================================================================================================================================================+
...
Content adapted from Luc Anselin Spatial Econometrics, filtered by ChatGPT.
...
ggplot(armed_clash_long, aes(x = value, 
  ...
  facet_wrap(~measure, scales = "free_x") +  # Facet by the 'measure' (FATALITIES or event_count)
...
```{r}
plot(armed_clash_myn_ppp, cex = 0.1, main="Spatial Distribution of Battles (Armed Clash) in Myanmar")
```
...
## 6.4 Extracting Study Areas

We can create a vector of the quarter time period and use the functions lapply() (or purrr::map()) to filter and select the data for each quarter, storing the results in a list:

```{r}
# 2021
q1_2021 <- armed_clash_sf %>% 
  filter(quarter == "Q1 2021") %>% 
  dplyr::select(event_type, geometry)

q2_2021 <- armed_clash_sf %>% 
  filter(quarter == "Q2 2021")  %>% 
  dplyr::select(event_type, geometry)
  
q3_2021 <- armed_clash_sf %>% 
  filter(quarter == "Q3 2021")  %>% 
  dplyr::select(event_type, geometry)
  
q4_2021 <- armed_clash_sf %>% 
  filter(quarter == "Q4 2021")  %>% 
  dplyr::select(event_type, geometry)

# 2022
q1_2022 <- armed_clash_sf %>% 
  filter(quarter == "Q1 2022")  %>% 
  dplyr::select(event_type, geometry)

q2_2022 <- armed_clash_sf %>% 
  filter(quarter == "Q2 2022")  %>% 
  dplyr::select(event_type, geometry)
  
q3_2022 <- armed_clash_sf %>% 
  filter(quarter == "Q3 2022")  %>% 
  dplyr::select(event_type, geometry)
  
q4_2022 <- armed_clash_sf %>% 
  filter(quarter == "Q4 2022")  %>% 
  dplyr::select(event_type, geometry)

# 2023
q1_2023 <- armed_clash_sf %>% 
  filter(quarter == "Q1 2023")  %>% 
  dplyr::select(event_type, geometry)

q2_2023 <- armed_clash_sf %>% 
  filter(quarter == "Q2 2023")  %>% 
  dplyr::select(event_type, geometry)
  
q3_2023 <- armed_clash_sf %>% 
  filter(quarter == "Q3 2023")  %>% 
  dplyr::select(event_type, geometry)
  
q4_2023 <- armed_clash_sf %>% 
  filter(quarter == "Q4 2023")  %>% 
  dplyr::select(event_type, geometry)

# 2024
q1_2024 <- armed_clash_sf %>% 
  filter(quarter == "Q1 2024")  %>% 
  dplyr::select(event_type, geometry)

q2_2024 <- armed_clash_sf %>% 
...
### 9.1.1 Clark and Evans Test: Q3 2022

```{r}
clarkevans.test(q3_2022_ppp.km, 
                correction="none", 
                clipregion="myanmar_states_owin", 
                alternative=c("two.sided"), 
                nsim=999)
...
### 9.1.2 Clark and Evans Test: Q4 2023

```{r}
clarkevans.test(q4_2023_ppp.km, 
                correction="none", 
                clipregion="myanmar_states_owin", 
                alternative=c("two.sided"), 
                nsim=999)
...
### 9.1.3 Clark and Evans Test: Q2 2024
...
From the bar chart we have generated above:
...
We can focus our analysis on the 2 Quarters with the most fatalities Q3 2022, Q4 2023 and Q2 2024 for 2nd Order Point Pattern Analysis

# 11.0 Analysing Spatial Point Process Using G-Function

## 11.1 Q3 2022

### 11.1.1 Computing G-function estimation

The code chunk below is used to compute G-function using *Gest()* of **spatat** package.

```{r}
G_Q3_2022 <- Gest(q3_2022_ppp.km, correction = "border")
plot(G_Q3_2022)
```
...
```{r}
G_Q3_2022.csr <- envelope(q3_2022_ppp, Gest, nsim=99)
```
...
### 11.2.1 Computing G-function estimation

```{r}
G_Q4_2023 <- Gest(q4_2023_ppp, correction = "border")
plot(G_Q4_2023)
```
...
```{r}
G_Q4_2023.csr <- envelope(q4_2023_ppp, Gest, nsim=99)
```
...
# 12.0 Analysing Spatial Point Process Using F-Function
...
### 12.1.1 Computing F-function estimation

The code chunk below is used to compute F-function using *Fest()* of **spatat** package.

```{r}
F_Q3_2022 <- Fest(q3_2022_ppp, correction = "best")
plot(F_Q3_2022)
```
...
```{r}
F_Q3_2022.csr <- envelope(q3_2022_ppp, Fest, nsim=99)
```
...
### 11.2.1 Computing F-function estimation

```{r}
F_Q4_2023 <- Fest(q4_2023_ppp, correction = "best")
plot(F_Q4_2023)
```
...
```{r}
F_Q4_2023.csr <- envelope(q4_2023_ppp, Fest, nsim=99)
```
...
# 13.0 Analysing Spatial Point Process Using K-Function
...
### 13.1.1 Computing K-function estimation

The code chunk below is used to compute K-function using *Kest()* of **spatat** package.

```{r}
K_Q3_2022 <- Kest(q3_2022_ppp, correction = "Ripley")
plot(K_Q3_2022, . -r ~ r, ylab= "K(d)-r", xlab = "d(m)")
```
...
Ho = The distribution of childcare services at Armed Conflict (Battles) are randomly distributed.

H1= The distribution of childcare services at Armed Conflict (Battles) are not randomly distributed.
...
```{r}
K_Q3_2022.csr <- envelope(q3_2022_ppp, Kest, nsim=30, rank =1 , glocal=TRUE)
```
...
### 13.2.1 Computing K-function estimation

```{r}
K_Q4_2023 <- Kest(q4_2024_ppp, correction = "Ripley")
plot(K_Q4_2023,. -r ~ r, 
     ylab= "K(d)-r", xlab = "d(m)", 
     xlim=c(0,1000))
```

### 13.2.2 Performing Complete Spatial Randomness Test

```{r}
K_Q4_2023.csr <- envelope(q4_2023_ppp, Kest, nsim=99, rank=1, glocal=TRUE)
```

```{r}
plot(K_Q4_2023.csr, . - r ~ r, 
     xlab="d", ylab="K(d)-r", xlim=c(0,500))
```

# 14.0 Analysing Spatial Point Process Using L-Function

K-function measures the number of events found up to a given distance of any particular event. In this section, you will learn how to compute K-function estimates by using [*Kest()*](https://rdrr.io/cran/spatstat/man/Kest.html) of **spatstat** package. You will also learn how to perform monta carlo simulation test using *envelope()* of spatstat package.

## 14.1 Q3 2022

### 14.1.1 Computing L-function estimation

The code chunk below is used to compute K-function using *Kest()* of **spatat** package.

```{r}
L_Q3_2022 <- Kest(q3_2022_ppp, correction = "Ripley")
plot(L_Q3_2022, . -r ~ r, ylab= "K(d)-r", xlab = "d(m)")
```
...
```{r}
L_Q3_2022.csr <- envelope(q3_2022_ppp, Lest, nsim=99, rank =1 , glocal=TRUE)
```
...
### 14.2.1 Computing F-function estimation

```{r}
L_Q4_2023 <- Kest(q4_2024_ppp, correction = "Ripley")
plot(L_Q4_2023,. -r ~ r, 
     ylab= "K(d)-r", xlab = "d(m)", 
     xlim=c(0,1000))
```
...
```{r}
L_Q4_2023.csr <- envelope(q4_2023_ppp, Lest, nsim=99, rank=1, glocal=TRUE)
```

```{r}
plot(L_Q4_2023.csr, . - r ~ r, 
     xlab="d", ylab="K(d)-r", xlim=c(0,500))
```
