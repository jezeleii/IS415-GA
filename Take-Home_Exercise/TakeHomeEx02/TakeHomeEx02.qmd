...
```{r}
# Clean province names in both datasets
clean_names <- function(column) {
  gsub("\\s+", " ", trimws(column))
}

# Apply clean_names to each value in the columns
...
# Identify mismatched province names
...
# Ensure that you manually verify if these mismatched names map correctly
lookup <- data.frame(
  province_en = prov_not_in_thai_sf,
  correct_province = prov_not_in_offenses_sf
)

# Replace mismatched province names in drug_offenses using the lookup table
...
proportion_cases_long <- proportion_cases %>% 
  pivot_longer(cols = everything(), 
               names_to = "types_of_drug_offenses", 
               values_to = "proportion") %>% 
  arrange(desc(proportion))

proportion_cases_long
```

### 1.3.3 Visualizing Proportion of Cases in Thailand 

```{r}
ggplot(proportion_cases_long, aes(x = reorder(types_of_drug_offenses, proportion), y = proportion)) +
  geom_col(fill = "red") +
  ...
  theme_minimal() +
  ylim(0, max(proportion_cases_long$proportion) + 10)
...
```{r}
write_rds(drug_offenses_sf, "data/rds/thai_drugs_sf.rds")
```

```{r}
thai_drugs_sf <- read_rds("data/rds/thai_drugs_sf.rds") %>% 
  st_transform(crs = 32648)
```
