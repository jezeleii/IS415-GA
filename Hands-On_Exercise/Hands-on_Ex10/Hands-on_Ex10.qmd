...
We reveal the extent of *mpsz_svy21* by using `st_bbox()` of the sf package.
...
## 3.2 Converting aspatial data frame into an sf object
...
We can plot LOG_SELLING_PRICE using the code chunk below:
...
The distribution is relatively less skewed after transformation
...
```{r}
corrplot(cor(condo_resale[, 1:23]), diag=FALSE,order="AOE",
...
### 5.5.1 Checking for multicollinearity
